...
Distributed as `@zren-zing/codegraph-qt` on npm; same binary serves as installer, indexer, and MCP server.
...
`src/installer/` is the entry point for `codegraph install` (and the bare `codegraph`/`npx @zren-zing/codegraph-qt` invocation). Architecture:
...
Released to npm and mirrored as [GitHub Releases](https://github.com/zren-zing/codegraph-qt/releases). `CHANGELOG.md` is the source of truth; GitHub Release notes are extracted from it.
