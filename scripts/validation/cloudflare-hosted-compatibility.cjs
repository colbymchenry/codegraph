...
createExtensionProject(author, 'hosted-compatible-20260922');
...
async function publicationSlot(){await new Promise(r=>setTimeout(r,Math.max(0,lastPublication+7500-Date.now())));lastPublication=Date.now();}
async function publish(version, engines = '>=1.6.0 <2', id = 'hosted-compatible-20260922', broken = false) {
...
async function graph(root) {
  ...
  try { return g.getNodesByKind('route').flatMap(n => g.getOutgoingEdges(n.id).filter(e => e.metadata?.synthesizedBy === 'hosted-compatible-20260922').map(e => ({ source: n.name, target: g.getNode(e.target)?.name, label: e.metadata.label }))); }
  finally { g.close(); }
}
...
(async () => {
  try {
    registry = process.env.HOSTED_REGISTRY;
    assert.match(registry || '', /^https:\/\/[a-z0-9.-]+\.workers\.dev$/);
    assert.equal((await (await fetch(registry+'/api/extensions/hosted-compatible-20260922')).json()).length,0,'Use a fresh isolated target or explicitly rename this owned fixture before another run');
    await cli(['test', author]);
    await cli(['pack', author, '--out', path.join(lab,'starter.cgext')]);
    ...
    await publish('20.0.0-beta.1'); await publish('9.0.0', '>=9'); await publish('1.0.0', '>=9', 'hosted-unavailable-20260922');
    await publicationSlot();
    const duplicate = await fetch(registry + '/api/publish', { method: 'POST', body: JSON.stringify(pinned.body) });
    assert.equal(duplicate.status, 400);
    const all = await (await fetch(registry + '/api/extensions/hosted-compatible-20260922')).json();
    ...
    if (registry.startsWith('https:')) assert.equal(await page.evaluate(() => isSecureContext), true);
    ...
    await page.getByRole('link', { name: 'hosted-compatible-20260922', exact: true }).click();
    ...
    const noChoice = await page.evaluate(async () => {
      try { await rpc('command', { action: 'resolve', id: 'hosted-unavailable-20260922', project: '1' }); return ''; } catch (e) { return e.message; }
    });
    assert.match(noChoice, /No compatible stable release/);
    await page.evaluate(() => rpc('command', { action: 'install', id: 'hosted-unavailable-20260922', project: '1' }));
    ...
    await page.evaluate(() => navigate('/extensions/hosted-unavailable-20260922'));
    ...
    await cli(['install', 'hosted-compatible-20260922', '--registry', registry, '--path', roots[2]]); await expectGraph(roots[2], '1.10.0');
    const headlessConfig = fs.readFileSync(path.join(roots[2], 'codegraph.json'), 'utf8');
    const exactFailure = await cli(['install', 'hosted-compatible-20260922', '--registry', registry, '--version', '9.0.0', '--path', roots[2]], 1);
    assert.match(exactFailure.stderr, /No compatible exact release 9.0.0/);
    await cli(['update', 'hosted-unavailable-20260922', '--registry', registry, '--path', roots[2]], 1);
    const none = await cli(['install', 'hosted-unavailable-20260922', '--registry', registry, '--path', roots[2]], 1);
    ...
    await cli(['install', 'hosted-compatible-20260922', '--registry', registry, '--version', '20.0.0-beta.1', '--path', roots[2]]);
    await expectGraph(roots[2], '20.0.0-beta.1');
    const previewConfig = fs.readFileSync(path.join(roots[2], 'codegraph.json'), 'utf8');
    const downgrade = await cli(['update', 'hosted-compatible-20260922', '--registry', registry, '--path', roots[2]], 1);
    ...
    await cli(['install', 'hosted-compatible-20260922', '--registry', registry, '--version', '1.9.0', '--path', roots[2]]); await expectGraph(roots[2], '1.9.0');
    checks.push('prerelease requires explicit exact pin; automatic update refuses downgrade; explicit lower pin remains exact');
    await publish('2.0.0', '>=1.6.0 <2', 'hosted-compatible-20260922', true);
    ...
    await page.getByText('CodeGraph connected', { exact: true }).waitFor();
    await page.evaluate(() => { state.project = '1'; navigate('/extensions/hosted-compatible-20260922'); return refreshSelections(); });
    ...
    await cli(['update', 'hosted-compatible-20260922', '--registry', registry, '--path', roots[2]]); await expectGraph(roots[2], '3.0.0');
    ...
    await cli(['install', registry + '/api/download/hosted-compatible-20260922/1.9.0', '--path', roots[2]]); await expectGraph(roots[2], '1.9.0');
    ...
    await cli(['install', 'hosted-compatible-20260922', '--registry', registry, '--version', '1.10.0+fixture.1', '--path', roots[2]]);
    ...
    await cli(['remove', 'hosted-compatible-20260922', '--path', roots[2]]);
    ...
    fs.writeFileSync(path.join(out, 'result.json'), JSON.stringify({ registry, checks, releases, commands, engineVersion: require('../../package.json').version, platform: process.platform, browser: browser.version() }, null, 2));
