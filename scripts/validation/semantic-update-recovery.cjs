...
const phases=['candidate_ready','before_commit','in_commit','committed'];
...
if(process.argv[2]==='--child'){
 ...
 if(action==='update'){
  ...
  fs.writeFileSync(path.join(root,'binding.py'),'cancel');await g.sync();
 ...
 const expected=phase==='committed'?'cancel':'receipt';assert.deepEqual(snapshot(root),[['dispatch',expected]]);assert.ok(reader.getStats().nodeCount>0);
 ...
 fs.writeFileSync(path.join(root,'keep.txt'),'unrelated user file');const inspect=await child(root,'inspect').done;assert.equal(inspect.exit,0,inspect.stderr);assert.deepEqual(snapshot(root),[['dispatch',expected]]);
 ...
 assert.equal((await child(root,'inspect').done).exit,0);assert.equal((await child(root,'retry').done).exit,0);assert.deepEqual(snapshot(root),[['dispatch','cancel']]);
