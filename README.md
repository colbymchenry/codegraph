...
| PostgreSQL | `.sql`, `.psql`, `.pgsql` | Initial support (schemas, tables and their `CREATE TABLE` columns, views/materialized views, functions/procedures, enums/domains, triggers, policies, indexes, sequences, extensions, foreign keys, relation dependencies, and routine calls; columns added later by `ALTER TABLE`, and string-literal routine bodies including dollar-quoted PL/pgSQL, are not parsed yet) |
