...
| **20+ Languages** | TypeScript, JavaScript, ArkTS, Python, Go, Rust, Java, C#, VB.NET, PHP, Ruby, C, C++, CUDA, Objective-C, Metal, Swift, Kotlin, Scala, Dart, Lua, Luau, R, Nix, Erlang, CFML, COBOL, Solidity, Terraform/OpenTofu, Svelte, Vue, Astro, Liquid, Pascal/Delphi, Clojure |
...
| Clojure | `.clj`, `.cljc`, `.cljs`, `.edn` | Full support (head-symbol dispatch: `defn`/`defn-`/`defmacro`/`defmulti`/`deftest` functions, `defmethod` methods, `defprotocol`/`definterface` interfaces, `defrecord`/`deftype` classes, `def`/`defonce`/`defstate` constants, `ns` namespaces with `:require`/`:import`/`:use` edges, alias-qualified and bare call edges, `#'var` quoted references, `^:private`/`defn-` visibility; Java interop statics excluded from call edges) |
