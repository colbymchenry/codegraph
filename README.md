...
| **20+ Languages** | TypeScript, JavaScript, ArkTS, Python, Go, Rust, Java, C#, VB.NET, PHP, Perl, Ruby, C, C++, CUDA, Objective-C, Metal, Swift, Kotlin, Scala, Dart, Lua, Luau, R, Nix, Erlang, CFML, COBOL, Solidity, Terraform/OpenTofu, Svelte, Vue, Astro, Liquid, Pascal/Delphi |
...
| Perl | `.pl`, `.pm`, `.t`, `.pod`, `.psgi`, `.cgi` | Full support (packages as namespaces with Perl's absolute, statement-scoped package rules — including block and block-local `package` forms; subroutines with qualified `Package::sub` names; `use`/`require` imports with pragmas filtered; `use parent`/`use base` inheritance edges, `qw()` lists split per parent; `use constant` names in both single and hash forms; package-level `our`/`my` variables with lexicals excluded; call edges for plain, ampersand, and `Class->method` calls, plus `Class->new` instantiation; `.t`/`.cgi` yielded to another language when a shebang names one) |
