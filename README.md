...
### Supercharge Claude Code, Cursor, Codex, OpenCode, Hermes Agent, Gemini, Antigravity, Kiro, GitHub Copilot, and Kimi Code with Semantic Code Intelligence
...
[![Kimi Code](https://img.shields.io/badge/Kimi_Code-supported-blueviolet.svg)](#supported-agents)
...
The installer will:
- Ask which agent(s) to configure — auto-detects installed ones from: **Claude Code**, **Cursor**, **Codex CLI**, **opencode**, **Hermes Agent**, **Gemini CLI**, **Antigravity IDE**, **Kiro**, **GitHub Copilot** (VS Code, Copilot CLI, JetBrains IDEs), **Kimi Code**
...
Restart your agent (Claude Code / Cursor / Codex CLI / opencode / Hermes Agent / Gemini CLI / Antigravity IDE / Kiro / Kimi Code / VS Code, the Copilot CLI, or your JetBrains IDE for GitHub Copilot) for the MCP server to load.
...
- **Kimi Code** — writes `mcpServers.codegraph` to `~/.kimi-code/mcp.json` (global) or `./.kimi-code/mcp.json` (project); Kimi Code infers stdio from the `command` field, so no `type` field is written. Start a new Kimi Code session for the MCP server to connect.
...
**Made for AI coding agents — Claude Code, Cursor, Codex CLI, opencode, Hermes Agent, Gemini CLI, Antigravity IDE, Kiro, GitHub Copilot, and Kimi Code**
