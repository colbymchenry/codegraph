...
- For OpenSpec v1.7 single-item validation, use `openspec validate "<item-name>" --type change --strict --json` or `--type spec`; do not use `openspec validate --change ...`
- After syncing a new capability delta, validate the resulting main spec and ensure it uses main-spec format (`# <capability> Specification`, `## Purpose`, `## Requirements`) with no delta headers like `## ADDED Requirements`
