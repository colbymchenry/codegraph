...
5. **Validate and show final status**
   ...
   **Pitfall:** `openspec validate` does not accept `--change`; validate a specific change by passing the change name as the positional argument plus `--type change`. If you need to inspect JSON from `openspec instructions` or `openspec status`, avoid piping directly into an interpreter in environments with command-approval/security scanning; write JSON to a temp file or inspect it with normal terminal output first.
