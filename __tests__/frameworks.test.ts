...
`;
  ...
  it('extracts a GoRoute with a block-body pageBuilder (real-world idiom)', () => {
    // Most real-world GoRouter code uses a block body for any non-trivial
    // wrapping (page transitions, auth gates, key plumbing). The capture
    // takes the first widget after `return`; the next hop (wrapper -> child
    // screen) is then followed by the standard Dart constructor-call edges.
    const src = `
final router = GoRouter(
  routes: [
    GoRoute(
...
`;
    const { nodes, references } = flutterRouterResolver.extract!('lib/router.dart', src);
    expect(nodes.map((n) => n.name)).toEqual(['/books/popular']);
    expect(references.map((r) => r.referenceName)).toEqual(['FadeTransitionPage']);
  });

  it('accepts a Dart constant (Routes.login) as the GoRoute path expression', () => {
    // Real-world Flutter apps centralize paths in `class Routes { static const
    // login = '/login'; … }` and reference them as `path: Routes.login`. The
    // extractor surfaces the identifier as the route name so the route → handler
    // edge still materializes; agents can read Routes.dart for the literal.
    const src = `
GoRoute(
  path: Routes.login,
...
`;
    const { nodes, references } = flutterRouterResolver.extract!('lib/routing/router.dart', src);
    expect(nodes.map((n) => n.name)).toEqual(['/Routes.login']);
    expect(references.map((r) => r.referenceName)).toEqual(['LoginScreen']);
  });

  it('captures the outer generic-typed page wrapper (FadeTransitionPage<dynamic>(…)) not a nested child', () => {
    // Real bookstore sample: pageBuilder returns FadeTransitionPage<dynamic>(child:
    // Builder(builder: (c) { return BookList(...); })). Without the optional
    // <…> allowance, the regex stopped at `FadeTransitionPage` then backtracked
    // and captured the inner `BookList`. The captured wrapper is fine — the
    // wrapper -> child edge is followed by the standard call graph.
    const src = `
GoRoute(
  path: '/books/popular',
  pageBuilder: (context, state) {
...
`;
  ...
  it('does not attribute a nested child GoRoute’s path to its parent ShellRoute', () => {
    // The bookstore sample shape: ShellRoute wraps a chain of GoRoutes. The
    // parent's `path:` / `pageBuilder:` must come from its OWN params, not
    // from the descendants reachable via routes: [...].
    const src = `
GoRouter(
  routes: [
    ShellRoute(
      builder: (context, state, child) {
        return ScaffoldWrap(child: child);
      },
      routes: [
        GoRoute(
          path: '/books/popular',
...
`;
    const { nodes, references } = flutterRouterResolver.extract!('lib/router.dart', src);
    // Only the inner GoRoute has a `path:`, so only one route node should
    // exist. The parent ShellRoute must NOT inherit the child's path.
    expect(nodes.map((n) => n.name)).toEqual(['/books/popular']);
    expect(references.map((r) => r.referenceName)).toEqual(['BooksScreen']);
  });

  it('extracts a MaterialApp route entry with a block-body builder', () => {
    const src = `
MaterialApp(
  routes: {
    '/profile': (ctx) {
...
`;
    const { nodes, references } = flutterRouterResolver.extract!('lib/app.dart', src);
    expect(nodes.map((n) => n.name)).toEqual(['/profile']);
    expect(references.map((r) => r.referenceName)).toEqual(['ProfileScreen']);
  });
