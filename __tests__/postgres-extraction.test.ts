...
describe('PostgreSQL extraction', () => {
  it('reports recovered syntax errors instead of silently marking the file clean', () => {
    const result = extract('CREATE TABLE broken (id bigint,,);');
  ...
  it('does not treat PostgreSQL range and multirange catalog types as user-defined', () => {
    const result = extract([
      'CREATE TABLE app.availability (',
      '  dates daterange, date_sets datemultirange,',
      '  integers int4range, integer_sets int4multirange,',
      '  bigints int8range, bigint_sets int8multirange,',
      '  numbers numrange, number_sets nummultirange,',
      '  timestamps tsrange, timestamp_sets tsmultirange,',
      '  zoned_times tstzrange, zoned_time_sets tstzmultirange',
      ');',
    ].join('\n'));

    expect(result.errors).toEqual([]);
    expect(result.unresolvedReferences.filter(
      (ref) => ref.referenceKind === POSTGRES_TYPE_REFERENCE_KIND
    )).toEqual([]);
  });

  it('emits sequence references only for literal first arguments through casts', () => {
    const result = extract([
      'CREATE TABLE app.events (',
      "  literal_id bigint DEFAULT nextval((('app.literal_seq'))::regclass),",
      "  cast_id bigint DEFAULT setval(CAST('app.cast_seq' AS regclass), 1),",
      "  current_id bigint DEFAULT currval('app.current_seq'::regclass),",
      "  concatenated_id bigint DEFAULT nextval(('app.foo_' || tenant)::regclass),",
      "  formatted_id bigint DEFAULT nextval(format('app.%s_seq', tenant)::regclass)",
      ');',
    ].join('\n'));

    expect(result.errors).toEqual([]);
    expect(result.unresolvedReferences
      .filter((ref) => ref.referenceKind === POSTGRES_SEQUENCE_REFERENCE_KIND)
      .map((ref) => ref.referenceName)
      .sort()
    ).toEqual(['app.cast_seq', 'app.current_seq', 'app.literal_seq']);
  });
  ...
  it('indexes renames, ALTER policy/enum/sequence state, and non-FK constraints', () => {
    const result = extract([
      'SET search_path = public;',
      "CREATE TYPE lease_status AS ENUM ('draft');",
      "ALTER TYPE lease_status ADD VALUE 'pending_signature' BEFORE 'completed';",
      "CREATE TABLE submission_tags (id bigint, label text, status lease_status, sequence_id bigint DEFAULT nextval('daily_application_counts_id_seq'::regclass),",
      '  CONSTRAINT submission_tags_pkey PRIMARY KEY (id),',
      '  CONSTRAINT submission_tags_label_key UNIQUE (label),',
      '  CONSTRAINT submission_tags_label_check CHECK (length(label) > 0));',
      'ALTER TABLE submission_tags RENAME TO tags;',
      'ALTER TABLE tags RENAME COLUMN label TO title;',
      'ALTER TABLE tags ALTER COLUMN status SET DATA TYPE lease_status USING status::text::lease_status;',
      'CREATE POLICY tags_read ON tags USING (true);',
      'ALTER POLICY tags_read ON tags USING (EXISTS (SELECT 1 FROM organizations));',
      'ALTER POLICY tags_read ON tags RENAME TO tags_select;',
      'CREATE SEQUENCE daily_application_counts_id_seq;',
      'ALTER SEQUENCE daily_application_counts_id_seq OWNED BY tags.id;',
      'ALTER TABLE tags ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY',
      '  (SEQUENCE NAME tags_id_seq);',
      'ALTER TABLE tags RENAME CONSTRAINT submission_tags_label_key TO tags_title_key;',
      'ALTER TABLE tags DROP CONSTRAINT submission_tags_pkey;',
    ].join('\n'));

    expect(result.errors).toEqual([]);
    const byQualifiedName = new Map(result.nodes.map((node) => [node.qualifiedName, node]));
    expect(byQualifiedName.get('public.tags')?.decorators).toEqual(expect.arrayContaining([
      'postgres:table',
      'postgres:renamed-relation',
    ]));
    expect(byQualifiedName.get('public.tags.title')?.decorators).toEqual(expect.arrayContaining([
      'postgres:column',
      'postgres:renamed-column',
    ]));
    expect(byQualifiedName.get('public.lease_status.pending_signature')?.decorators).toEqual(
      expect.arrayContaining(['postgres:enum-value', 'postgres:alter-enum-add-value'])
    );
    expect(byQualifiedName.get('public.tags.tags_select')?.decorators).toEqual(
      expect.arrayContaining(['postgres:policy', 'postgres:renamed-policy'])
    );
    expect(result.nodes.find((node) =>
      node.qualifiedName === 'public.tags.tags_read' &&
      node.decorators?.includes('postgres:alter-policy')
    )).toBeTruthy();
    expect(result.nodes.filter((node) => node.decorators?.includes('postgres:constraint')))
      .toEqual(expect.arrayContaining([
        expect.objectContaining({ name: 'submission_tags_pkey' }),
        expect.objectContaining({ name: 'submission_tags_label_key' }),
        expect.objectContaining({ name: 'submission_tags_label_check' }),
      ]));
    expect(byQualifiedName.get('public.tags_id_seq')?.decorators).toEqual(
      expect.arrayContaining(['postgres:sequence', 'postgres:identity-sequence'])
    );
    expect(result.nodes.find((node) =>
      node.name === 'daily_application_counts_id_seq' &&
      node.decorators?.includes('postgres:sequence-ownership')
    )).toBeTruthy();

    const statusField = byQualifiedName.get('public.submission_tags.status')!;
    expect(result.unresolvedReferences).toEqual(expect.arrayContaining([
      expect.objectContaining({
        fromNodeId: statusField.id,
        referenceKind: POSTGRES_TYPE_REFERENCE_KIND,
        referenceName: 'lease_status',
      }),
      expect.objectContaining({
        fromNodeId: byQualifiedName.get('public.submission_tags.sequence_id')!.id,
        referenceKind: POSTGRES_SEQUENCE_REFERENCE_KIND,
        referenceName: 'daily_application_counts_id_seq',
      }),
    ]));
    const typeDelta = result.nodes.find((node) =>
      node.decorators?.includes('postgres:alter-table-column-type')
    )!;
    expect(result.unresolvedReferences.some((ref) =>
      ref.fromNodeId === typeDelta.id &&
      ref.referenceKind === POSTGRES_TYPE_REFERENCE_KIND &&
      ref.referenceName === 'lease_status'
    )).toBe(true);
    const sequenceOwnership = result.nodes.find((node) =>
      node.decorators?.includes('postgres:sequence-ownership')
    )!;
    expect(result.unresolvedReferences.some((ref) =>
      ref.fromNodeId === sequenceOwnership.id &&
      ref.referenceKind === POSTGRES_SEQUENCE_REFERENCE_KIND &&
      ref.referenceName === 'public.daily_application_counts_id_seq'
    )).toBe(true);

    const alteredPolicy = result.nodes.find((node) =>
      node.decorators?.includes('postgres:alter-policy')
    )!;
    expect(result.unresolvedReferences
      .filter((ref) => ref.fromNodeId === alteredPolicy.id)
      .map((ref) => ref.referenceName)
      .sort()
    ).toEqual(['organizations', 'tags']);

    const drop = result.nodes.find((node) =>
      node.decorators?.includes(POSTGRES_DROP_CONSTRAINT_DECORATOR)
    )!;
    expect(decodePostgresDropConstraintDescriptor(drop.decorators)).toEqual({
      table: 'public.tags',
      constraintName: 'submission_tags_pkey',
    });

    const rename = result.nodes.find((node) =>
      node.decorators?.includes(POSTGRES_RENAME_CONSTRAINT_DECORATOR)
    )!;
    expect(decodePostgresRenameConstraintDescriptor(rename.decorators)).toEqual({
      table: 'public.tags',
      sourceConstraint: 'submission_tags_label_key',
      targetConstraint: 'tags_title_key',
    });
  });

  it('records ordered table and view drops as relation lifecycle facts', () => {
    const result = extract([
      'SET search_path = app;',
      'DROP TABLE IF EXISTS users, audit.events CASCADE;',
      'DROP MATERIALIZED VIEW IF EXISTS user_rollup;',
    ].join('\n'));

    expect(result.errors).toEqual([]);
    const drops = result.nodes.filter((node) =>
      node.decorators?.includes(POSTGRES_DROP_RELATION_DECORATOR)
    );
    expect(drops.map((node) => decodePostgresDropRelationDescriptor(node.decorators)))
      .toEqual(expect.arrayContaining([
        { relationName: 'app.users', relationKind: 'table' },
        { relationName: 'audit.events', relationKind: 'table' },
        { relationName: 'app.user_rollup', relationKind: 'materialized-view' },
      ]));
    expect(result.unresolvedReferences.filter((ref) =>
      drops.some((drop) => drop.id === ref.fromNodeId)
    ).map((ref) => ref.referenceName).sort()).toEqual([
      'app.user_rollup',
      'app.users',
      'audit.events',
    ]);
  ...
  it('indexes static SQL regions inside PL/pgSQL routines and DO blocks', () => {
    const result = extract([
      ...
      'DO $$',
      'BEGIN',
      '  UPDATE app.job_state SET complete = true;',
      'END',
      '$$;',
    ].join('\n'));

    const routine = result.nodes.find((node) => node.qualifiedName === 'app.run_job')!;
    const doBlock = result.nodes.find((node) => node.decorators?.includes('postgres:do-block'))!;
    expect(routine).toBeTruthy();
    expect(doBlock).toBeTruthy();
    expect(result.unresolvedReferences
      .filter((ref) => ref.fromNodeId === routine.id)
      .map((ref) => `${ref.referenceKind}:${ref.referenceName}`)
      .sort()
    ).toEqual(['calls:app.hidden_call', 'references:app.hidden_table']);
    expect(result.unresolvedReferences
      .filter((ref) => ref.fromNodeId === doBlock.id)
      .map((ref) => `${ref.referenceKind}:${ref.referenceName}`)
    ).toEqual(['references:app.job_state']);
  });
