...
describe('CODEGRAPH_MCP_TOOLS allowlist', () => {
  ...
  it('exposes explore + the Vue component tool by default when unset', () => {
    delete process.env[ENV];
    // FORK default set (see DEFAULT_MCP_TOOLS): explore (verbatim source
    // grouped by file) + codegraph_component (the Vue component contract).
    // node/search/callers/callees/impact/files/status stay defined and
    // executable but unlisted; CODEGRAPH_MCP_TOOLS re-enables them.
    expect(listed()).toEqual(['codegraph_component', 'codegraph_explore']);
  });
