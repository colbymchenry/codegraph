...
// The viewer is deferred for this release. Reject both former spellings before
// startup/relaunch or Commander help processing; no environment opt-in.
const releaseArgs = process.argv.slice(2).filter(arg => !['--color', '--no-color'].includes(arg));
const deferredCommand = releaseArgs[0] === 'help' ? releaseArgs[1] : releaseArgs[0];
if (deferredCommand === 'ui' || deferredCommand === 'web') {
  process.stderr.write(`error: unknown command '${deferredCommand}' (viewer commands are not available in this release)\n`);
  process.exit(1);
}
...
// The standalone viewer commands (`ui` and `web`) are deferred. Do not register
// them here or behind a runtime environment switch. Extension companion/connect
// commands have their own lifecycle and remain available below.
