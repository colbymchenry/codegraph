...
export const flutterRouterResolver: FrameworkResolver = {
  ...
  extract(filePath, content) {
    ...
    // MaterialApp(... routes: { '/foo': (ctx) => FooScreen(), '/bar': (ctx) => BarScreen() })
    ...
    if (materialRoutesBlock && materialRoutesBlock[1]) {
      ...
      const entryRegex = /['"]([^'"]+)['"]\s*:\s*\([^)]*\)\s*=>\s*(?:const\s+)?(\w+)\s*\(/g;
      let em: RegExpExecArray | null;
      while ((em = entryRegex.exec(body)) !== null) {
        const [, path, handler] = em;
...
function extractGoRouter(
  ...
  while (i < n) {
    ...
    const body = safe.slice(openParen + 1, closeAt);

    // path: '...'
    let ownPath: string | null = null;
    const pathMatch = body.match(/\bpath\s*:\s*['"]([^'"]*)['"]/);
    if (pathMatch) ownPath = pathMatch[1] ?? '';

    // builder or pageBuilder: (c, s) => XScreen( ... )
    let handler: string | null = null;
    const handlerMatch = body.match(/\b(?:builder|pageBuilder)\s*:\s*\([^)]*\)\s*=>\s*(?:const\s+)?(\w+)\s*\(/);
    if (handlerMatch) handler = handlerMatch[1] ?? null;
