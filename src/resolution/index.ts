...
import { resolveViaImport, resolvePhpImportedStaticCall, resolveJvmImport, extractImportMappings, extractReExports, loadCppIncludeDirs, isPhpIncludePathRef, isCobolCopybookRef, isNixPathImportRef, isBoundToOutOfRepoImport, isExternalGleamReference, isGleamPreludeReference, clearImportResolverMemos, resolveImportPath } from './import-resolver';
...
export class ReferenceResolver {
  ...
  private resolveGleamErlangFfi(
    ...
    const targets = this.context
      .getNodesByQualifiedName(`${ffi.module}::${ffi.function}/${ffi.arity}`)
      .filter(
        (node) =>
  ...
  resolveAndPersist(
    ...
    if (result.unresolved.length > 0) {
      const cleanup = ReferenceResolver.partitionUnresolvedCleanup(this.nonDeferredFailures(result.unresolved));
  ...
  async resolveAndPersistBatched(
    ...
    while (batch.length > 0 && inFlight) {
      ...
      const cleanup = ReferenceResolver.partitionUnresolvedCleanup(failures);
      for (let i = 0; i < cleanup.failed.byRowId.length; i += PERSIST_CHUNK) {
