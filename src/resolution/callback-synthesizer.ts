...
import { protobufContractEdges } from './protobuf-contract-synthesizer';
...
export const SYNTH_PASSES: SynthPassDef[] = [
  ...
  // A `.proto` field to the generated code implementing it in each target
  // language. Gated on the project having protos at all; the join is
  // stem-to-stem, so a repo with no generated peers is a no-op.
  { name: 'protobufContractEdges', gate: (has) => has('proto'), run: (_q, c, y) => protobufContractEdges(c, y) },
];
