...
export interface HdlSemanticResult {
  schemaVersion: 1;
  provenance: { frontend: 'slang'; version: string; executableSha256: string;
...
export async function analyzeHdlSemantics(root: string, options: HdlSemanticOptions,
  ...
  const facts = importSlangSemantics(result.ast, result.sourceRoot);
  ...
  const selected = matches.slice(0, limit).map(fact => {
    ...
    if (!linked || source.column === null || source.column < 1 || source.column > Buffer.byteLength(line) + 1 || !line.includes(fact.name)) return fact;
    ...
    const nodes = byFile.get(source.file)!.filter(node => node.name === fact.name && node.startLine === source.line &&
      (fact.kind === 'parameter' ? node.kind === 'constant' : node.kind === 'field' && !!node.decorators?.includes('hdl:port')));
    return nodes.length === 1 ? { ...fact, sourceNodeId: nodes[0]!.id } : fact;
  });
  return { schemaVersion: 1, provenance: { frontend: 'slang', version: result.version,
