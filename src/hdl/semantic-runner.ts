...
const RUNNER_VERSION = 'slang-snapshot-v1';
...
function resolveExecutable(name: string): string {
  ...
  for (const candidate of candidates) {
    try { fs.accessSync(candidate, fs.constants.X_OK); const real = fs.realpathSync(candidate); if (fs.statSync(real).isFile()) return real; } catch { /* try next PATH entry */ }
  }
...
export async function runSlangSemantics(projectRoot: string, options: RunSlangOptions): Promise<RunSlangResult> {
  ...
  const executable=resolveExecutable(options.executable), executableSha256=await executableHash(executable);
  const before=collect(root,profile,signal,deadline);
  const base=process.env.CODEGRAPH_SEMANTIC_TMPDIR||path.join(os.homedir(),'storage','codegraph-semantic-runs');
  ...
  try {
    ...
    const version=(await run(executable,['--version'],sourceRoot,Math.min(remaining(),5000),signal)).stdout.trim();
    if(!/^slang version 11\.0(?:\.\d+)?(?:\+[A-Za-z0-9]+)?(?:\s.*)?$/.test(version))throw new Error(`Unsupported slang schema family: ${version.slice(0,200)}`);
    ...
    const output=await run(executable,args,sourceRoot,remaining(),signal,astFile);
    ...
    const provenance={configurationFingerprint:profile.fingerprint,hashes:before.hashes,missing:before.missing,version,executableSha256,top,parameters,allowUseBeforeDeclare,languageStandard,compilationUnitMode:'separate' as const,runnerVersion:RUNNER_VERSION,compilerLimits:COMPILER_LIMITS};
    return {ast,sourceRoot,profileName:profile.name,configurationFingerprint:profile.fingerprint,fingerprint:digest(JSON.stringify(provenance)),version,executableSha256,
