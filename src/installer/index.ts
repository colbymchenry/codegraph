 ...
 * Hermes Agent, Gemini CLI, Antigravity IDE, Kiro, Kimi Code, and
...
export async function runUninstaller(opts: RunUninstallerOptions): Promise<void> {
  ...
  if (opts.location) {
    ...
    const sel = await clack.select({
      ...
      options: [
        { value: 'global' as const, label: 'All projects (global)', hint: '~/.claude, ~/.cursor, ~/.codex, ~/.config/opencode, ~/.hermes, ~/.gemini, ~/.kiro, ~/.kimi-code, ~/.copilot, ~/.config/github-copilot' },
        { value: 'local'  as const, label: 'Just this project (local)', hint: './.claude, ./.cursor, ./.vscode, ./opencode.jsonc, ./.gemini, ./.kiro, ./.kimi-code' },
      ],
      initialValue: 'global' as const,
    });
    if (clack.isCancel(sel)) {
