 ...
 * Hermes Agent, Gemini CLI, Antigravity IDE, jcode).
...
export async function runUninstaller(opts: RunUninstallerOptions): Promise<void> {
  ...
  if (opts.location) {
    ...
    const sel = await clack.select({
      ...
      options: [
        { value: 'global' as const, label: 'All projects (global)', hint: '~/.claude, ~/.cursor, ~/.codex, ~/.config/opencode, ~/.hermes, ~/.gemini, ~/.kiro, ~/.jcode' },
        { value: 'local'  as const, label: 'Just this project (local)', hint: './.claude, ./.cursor, ./opencode.jsonc, ./.gemini, ./.kiro, ./.jcode' },
      ],
      initialValue: 'global' as const,
    });
    if (clack.isCancel(sel)) {
