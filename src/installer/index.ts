...
export async function runUninstaller(opts: RunUninstallerOptions): Promise<void> {
  ...
  if (opts.location) {
    ...
    const sel = await clack.select({
      ...
      options: [
        { value: 'global' as const, label: 'All projects (global)', hint: '~/.claude, ~/.cursor, ~/.codex, ~/.config/opencode, ~/.hermes, ~/.kimi-code' },
        { value: 'local'  as const, label: 'Just this project (local)', hint: './.claude, ./.cursor, ./opencode.jsonc, ./.kimi-code' },
