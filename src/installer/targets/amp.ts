...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ...
    : path.join(os.homedir(), '~/.config/agents/skills'.replace(/^~\//, ''));
}
...
class AmpTarget implements AgentTarget {
  readonly id = 'amp' as const;
  readonly displayName = 'Amp / Kimi / Replit / Universal';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Amp / Kimi / Replit / Universal for MCP changes to take effect.'
      ],
    };
  }
...
export const ampTarget: AgentTarget = new AmpTarget();
