...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'mux.mux/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'mux.mux/skills')
    : path.join(os.homedir(), '~/.mux/skills'.replace(/^~\//, ''));
}
...
class MuxTarget implements AgentTarget {
  readonly id = 'mux' as const;
  readonly displayName = 'Mux';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Mux for MCP changes to take effect.'
      ],
    };
  }
...
export const muxTarget: AgentTarget = new MuxTarget();
