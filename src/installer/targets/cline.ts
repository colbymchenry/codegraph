...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ...
    : path.join(os.homedir(), '~/.agents/skills'.replace(/^~\//, ''));
}
...
class ClineTarget implements AgentTarget {
  readonly id = 'cline' as const;
  readonly displayName = 'Cline / Dexto / Warp';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Cline / Dexto / Warp for MCP changes to take effect.'
      ],
    };
  }
...
export const clineTarget: AgentTarget = new ClineTarget();
