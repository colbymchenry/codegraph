...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'zencoder.zencoder/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'zencoder.zencoder/skills')
    : path.join(os.homedir(), '~/.zencoder/skills'.replace(/^~\//, ''));
}
...
class ZencoderTarget implements AgentTarget {
  readonly id = 'zencoder' as const;
  readonly displayName = 'Zencoder';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Zencoder for MCP changes to take effect.'
      ],
    };
  }
...
export const zencoderTarget: AgentTarget = new ZencoderTarget();
