...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), '.augment/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), '.augment/skills')
    : path.join(os.homedir(), '~/.augment/skills'.replace(/^~\//, ''));
}
...
class AugmentTarget implements AgentTarget {
  readonly id = 'augment' as const;
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Augment for MCP changes to take effect.'
      ],
    };
  }
...
export const augmentTarget: AgentTarget = new AugmentTarget();
