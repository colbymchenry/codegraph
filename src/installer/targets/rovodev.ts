...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'rovodev.rovodev/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'rovodev.rovodev/skills')
    : path.join(os.homedir(), '~/.rovodev/skills'.replace(/^~\//, ''));
}
...
class RovodevTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Rovo Dev for MCP changes to take effect.'
      ],
    };
  }
