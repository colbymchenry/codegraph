...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'openhands.openhands/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'openhands.openhands/skills')
    : path.join(os.homedir(), '~/.openhands/skills'.replace(/^~\//, ''));
}
...
class OpenhandsTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart OpenHands for MCP changes to take effect.'
      ],
    };
  }
...
export const openhandsTarget: AgentTarget = new OpenhandsTarget();
