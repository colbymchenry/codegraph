...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'pi.pi/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'pi.pi/skills')
    : path.join(os.homedir(), '~/.pi/agent/skills'.replace(/^~\//, ''));
}
...
class PipiTarget implements AgentTarget {
  readonly id = 'pipi' as const;
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Pi for MCP changes to take effect.'
      ],
    };
  }
...
export const pipiTarget: AgentTarget = new PipiTarget();
