...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'command-code.commandcode/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'command-code.commandcode/skills')
    : path.join(os.homedir(), '~/.commandcode/skills'.replace(/^~\//, ''));
}
...
class CommandCodeTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Command Code for MCP changes to take effect.'
      ],
    };
  }
...
export const commandCodeTarget: AgentTarget = new CommandCodeTarget();
