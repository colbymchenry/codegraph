...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), '.continue/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), '.continue/skills')
    : path.join(os.homedir(), '~/.continue/skills'.replace(/^~\//, ''));
}
...
class ContinueTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Continue for MCP changes to take effect.'
      ],
    };
  }
