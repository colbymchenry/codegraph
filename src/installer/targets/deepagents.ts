...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'deepagents.agents/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'deepagents.agents/skills')
    : path.join(os.homedir(), '~/.deepagents/agent/skills'.replace(/^~\//, ''));
}
...
class DeepagentsTarget implements AgentTarget {
  readonly id = 'deepagents' as const;
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Deep Agents for MCP changes to take effect.'
      ],
    };
  }
...
export const deepagentsTarget: AgentTarget = new DeepagentsTarget();
