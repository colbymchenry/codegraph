...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'trae-cn.trae/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'trae-cn.trae/skills')
    : path.join(os.homedir(), '~/.trae-cn/skills'.replace(/^~\//, ''));
}
...
class TraeCnTarget implements AgentTarget {
  readonly id = 'trae-cn' as const;
  readonly displayName = 'Trae CN';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Trae CN for MCP changes to take effect.'
      ],
    };
  }
...
export const traeCnTarget: AgentTarget = new TraeCnTarget();
