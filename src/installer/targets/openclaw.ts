...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'openclawskills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'openclawskills')
    : path.join(os.homedir(), '~/.openclaw/skills'.replace(/^~\//, ''));
}
...
class OpenclawTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart OpenClaw for MCP changes to take effect.'
      ],
    };
  }
