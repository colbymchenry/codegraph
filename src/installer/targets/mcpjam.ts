...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'mcpjam.mcpjam/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'mcpjam.mcpjam/skills')
    : path.join(os.homedir(), '~/.mcpjam/skills'.replace(/^~\//, ''));
}
...
class McpjamTarget implements AgentTarget {
  readonly id = 'mcpjam' as const;
  readonly displayName = 'MCPJam';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart MCPJam for MCP changes to take effect.'
      ],
    };
  }
...
export const mcpjamTarget: AgentTarget = new McpjamTarget();
