...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'junie.junie/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'junie.junie/skills')
    : path.join(os.homedir(), '~/.junie/skills'.replace(/^~\//, ''));
}
...
class JunieTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Junie for MCP changes to take effect.'
      ],
    };
  }
...
export const junieTarget: AgentTarget = new JunieTarget();
