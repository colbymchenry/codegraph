...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'goose.goose/skills'.replace(/\/skills$/, ''))
    : path.join(os.homedir(), '~/.config/goose/skills'.replace(/^~\//, '').replace(/\/skills$/, ''));
  return targetPath;
}

function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'goose.goose/skills')
    : path.join(os.homedir(), '~/.config/goose/skills'.replace(/^~\//, ''));
}
...
class GooseTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Goose for MCP changes to take effect.'
      ],
    };
  }
...
export const gooseTarget: AgentTarget = new GooseTarget();
