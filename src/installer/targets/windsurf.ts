...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'windsurf.windsurf/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'windsurf.windsurf/skills')
    : path.join(os.homedir(), '~/.codeium/windsurf/skills'.replace(/^~\//, ''));
}
...
class WindsurfTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Windsurf for MCP changes to take effect.'
      ],
    };
  }
