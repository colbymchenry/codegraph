...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), '.aider-desk/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), '.aider-desk/skills')
    : path.join(os.homedir(), '~/.aider-desk/skills'.replace(/^~\//, ''));
}
...
class AiderTarget implements AgentTarget {
  readonly id = 'aider' as const;
  readonly displayName = 'Aider Desk';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Aider Desk for MCP changes to take effect.'
      ],
    };
  }
...
export const aiderTarget: AgentTarget = new AiderTarget();
