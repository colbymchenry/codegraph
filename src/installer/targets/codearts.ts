...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'codeartsdoer/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'codeartsdoer/skills')
    : path.join(os.homedir(), '~/.codeartsdoer/skills'.replace(/^~\//, ''));
}
...
class CodeartsTarget implements AgentTarget {
  readonly id = 'codearts' as const;
  readonly displayName = 'CodeArts Agent';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart CodeArts Agent for MCP changes to take effect.'
      ],
    };
  }
...
export const codeartsTarget: AgentTarget = new CodeartsTarget();
