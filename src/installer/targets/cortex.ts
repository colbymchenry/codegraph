...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'cortex.cortex/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'cortex.cortex/skills')
    : path.join(os.homedir(), '~/.snowflake/cortex/skills'.replace(/^~\//, ''));
}
...
class CortexTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Cortex Code for MCP changes to take effect.'
      ],
    };
  }
...
export const cortexTarget: AgentTarget = new CortexTarget();
