...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'droid.factory/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'droid.factory/skills')
    : path.join(os.homedir(), '~/.factory/skills'.replace(/^~\//, ''));
}
...
class DroidTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Droid for MCP changes to take effect.'
      ],
    };
  }
...
export const droidTarget: AgentTarget = new DroidTarget();
