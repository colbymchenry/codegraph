...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'tabnine-cli.tabnine/agent/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'tabnine-cli.tabnine/agent/skills')
    : path.join(os.homedir(), '~/.tabnine/agent/skills'.replace(/^~\//, ''));
}
...
class TabnineTarget implements AgentTarget {
  readonly id = 'tabnine' as const;
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Tabnine CLI for MCP changes to take effect.'
      ],
    };
  }
...
export const tabnineTarget: AgentTarget = new TabnineTarget();
