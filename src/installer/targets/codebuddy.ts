...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'codebuddy.codebuddy/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'codebuddy.codebuddy/skills')
    : path.join(os.homedir(), '~/.codebuddy/skills'.replace(/^~\//, ''));
}
...
class CodebuddyTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart CodeBuddy for MCP changes to take effect.'
      ],
    };
  }
