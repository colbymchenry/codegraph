...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'kilo.kilocode/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'kilo.kilocode/skills')
    : path.join(os.homedir(), '~/.kilocode/skills'.replace(/^~\//, ''));
}
...
class KilocodeTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Kilo Code for MCP changes to take effect.'
      ],
    };
  }
...
export const kilocodeTarget: AgentTarget = new KilocodeTarget();
