...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'roo.roo/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'roo.roo/skills')
    : path.join(os.homedir(), '~/.roo/skills'.replace(/^~\//, ''));
}
...
class RooTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Roo Code for MCP changes to take effect.'
      ],
    };
  }
...
export const rooTarget: AgentTarget = new RooTarget();
