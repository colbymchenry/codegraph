...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'bob/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'bob/skills')
    : path.join(os.homedir(), '~/.bob/skills'.replace(/^~\//, ''));
}
...
class BobTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart IBM Bob for MCP changes to take effect.'
      ],
    };
  }
