...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'qwen-code.qwen/skills'.replace(/\/skills$/, ''))
    : path.join(os.homedir(), '~/.qwen/skills'.replace(/^~\//, '').replace(/\/skills$/, ''));
  return targetPath;
}

function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'qwen-code.qwen/skills')
    : path.join(os.homedir(), '~/.qwen/skills'.replace(/^~\//, ''));
}
...
class QwenTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Qwen Code for MCP changes to take effect.'
      ],
    };
  }
