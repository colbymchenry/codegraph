...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'crush.crush/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'crush.crush/skills')
    : path.join(os.homedir(), '~/.config/crush/skills'.replace(/^~\//, ''));
}
...
class CrushTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Crush for MCP changes to take effect.'
      ],
    };
  }
...
export const crushTarget: AgentTarget = new CrushTarget();
