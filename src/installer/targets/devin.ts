...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'devin.devin/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'devin.devin/skills')
    : path.join(os.homedir(), '~/.config/devin/skills'.replace(/^~\//, ''));
}
...
class DevinTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Devin for Terminal for MCP changes to take effect.'
      ],
    };
  }
