...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'trae.trae/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'trae.trae/skills')
    : path.join(os.homedir(), '~/.trae/skills'.replace(/^~\//, ''));
}
...
class TraeTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Trae for MCP changes to take effect.'
      ],
    };
  }
