...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'iflow-cli.iflow/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'iflow-cli.iflow/skills')
    : path.join(os.homedir(), '~/.iflow/skills'.replace(/^~\//, ''));
}
...
class IflowTarget implements AgentTarget {
  readonly id = 'iflow' as const;
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart iFlow CLI for MCP changes to take effect.'
      ],
    };
  }
...
export const iflowTarget: AgentTarget = new IflowTarget();
