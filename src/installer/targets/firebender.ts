...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'firebender.agents/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'firebender.agents/skills')
    : path.join(os.homedir(), '~/.firebender/skills'.replace(/^~\//, ''));
}
...
class FirebenderTarget implements AgentTarget {
  readonly id = 'firebender' as const;
  readonly displayName = 'Firebender';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Firebender for MCP changes to take effect.'
      ],
    };
  }
...
export const firebenderTarget: AgentTarget = new FirebenderTarget();
