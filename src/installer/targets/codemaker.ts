...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'codemaker.codemaker/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'codemaker.codemaker/skills')
    : path.join(os.homedir(), '~/.codemaker/skills'.replace(/^~\//, ''));
}
...
class CodemakerTarget implements AgentTarget {
  readonly id = 'codemaker' as const;
  readonly displayName = 'Codemaker';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Codemaker for MCP changes to take effect.'
      ],
    };
  }
...
export const codemakerTarget: AgentTarget = new CodemakerTarget();
