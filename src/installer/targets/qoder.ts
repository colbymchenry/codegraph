 ...
 *
 * Two Qoder-specific choices in this body:
 *
 *   1. **YAML frontmatter `trigger: always_on`.** Without it Qoder
 *      classifies the rule as "Model Decision" (agent decides per-turn
 *      whether to consult it), which defeats the whole purpose — we
 *      need this guidance in front of the agent on EVERY turn so it
 *      reaches for `codegraph_explore` before its built-in repo/code
 *      search tools. `always_on` maps to Qoder's "Always Apply".
 *
 *   2. **A streamlined English playbook, not the full
 *      `SERVER_INSTRUCTIONS`.** Every other target's instruction file
 *      (CLAUDE.md / AGENTS.md / GEMINI.md) stays minimal because those
 *      agents ALSO receive the full playbook via MCP
 *      `initialize.instructions`. Qoder's client does NOT consume that
 *      field, so we need a rule file — but the full playbook had
 *      several issues: (a) unconditional phrasing conflicted with
 *      Qoder's own system-level `SearchCodebase`-first rule; (b) "Trust
 *      — don't re-verify" discouraged legitimate verification; (c) "ONE
 *      call usually answers" over-promised; (d) Shell fallback ignored
 *      Qoder's stripped-PATH launch scenario. This version fixes all of
 *      those: conditional opening ("For indexed code"), explicit "When
 *      to use / When not to" triage, softened trust guidance ("verify
 *      with Read if contradictory"), and Qoder-aware shell fallback
 *      caveat. Total size ~1.8k chars, well under Qoder's 100k rule
 *      budget.
 */
const QODER_RULE_BODY = `# CodeGraph

For **indexed code** (project root has \`.codegraph/\`), prefer \`codegraph_explore\` for structural/flow questions — it returns the full source, call paths, and impact radius in one call, replacing multiple Read/Grep rounds. Index lags file writes by ~1s; for unindexed content (configs, docs, \`.qoder/\`, Markdown, etc.), use Read/Grep directly.

## When to use / When not to

**Use \`codegraph_explore\`:**
- Understanding code logic: "how does X work", "X's call chain"
- Finding definitions/references: "where is X defined", "who calls X"
- Exploring areas: "this module's structure", "relationship between X and Y"
- Reading a named symbol/file (with call paths and impact radius attached)

**Use Read / Grep / SearchCodebase directly:**
- Config files, Markdown, JSON, \`.qoder/\` content (not indexed by codegraph)
- User-attached or explicitly specified file paths
- Searching memory/knowledge base content
- After two consecutive \`codegraph_explore\` calls return empty/insufficient → fallback immediately

## How to query

Pass a natural-language question or symbol names (e.g., \`mutateElement renderScene\`) to \`codegraph_explore\`. It returns line-numbered source grouped by file (usable as Read output) plus call paths and impact radius. Overloaded symbols return all matching definitions in one call. Need more? Call again with more specific symbol names.

## Staleness & Safety

- **Check the staleness banner.** If the response starts with "⚠️ Some files referenced below were edited since the last index sync…", the listed files need re-indexing — use Read for those. Files not listed are fresh; codegraph results are trustworthy.
- **"⚠️ CodeGraph auto-sync is DISABLED…"** → index frozen; use Read for everything.
- Index lags writes by ~1s; prefer Read for recently edited files.
- Codegraph results are AST-based and generally trustworthy; if they contradict the context, verify with Read.

## Limitations

- No \`.codegraph/\` in project → don't use codegraph; use built-in tools. You may mention \`codegraph init\` but don't run it yourself.
- Cross-file resolution is best-effort; dynamic dispatch has coverage gaps (silent empty when no static edges).
- No correctness validation — that's the compiler/test/linter's job.

## Shell fallback

When MCP is unavailable: \`codegraph explore "<query>"\` (requires binary in PATH; Qoder launched from desktop may have incomplete PATH — use Read/Grep instead).
`;
...
function writeQoderRuleEntry(): WriteResult['files'][number] {
  ...
  if (fs.existsSync(file)) {
    ...
    atomicWriteFileSync(file, desired);
