...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'neovate.neovate/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'neovate.neovate/skills')
    : path.join(os.homedir(), '~/.neovate/skills'.replace(/^~\//, ''));
}
...
class NeovateTarget implements AgentTarget {
  readonly id = 'neovate' as const;
  readonly displayName = 'Neovate';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Neovate for MCP changes to take effect.'
      ],
    };
  }
...
export const neovateTarget: AgentTarget = new NeovateTarget();
