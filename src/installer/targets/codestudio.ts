...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'codestudio.codestudio/skills'.replace(/\/skills$/, ''))
    : path.join(os.homedir(), '~/.codestudio/skills'.replace(/^~\//, '').replace(/\/skills$/, ''));
  return targetPath;
}

function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'codestudio.codestudio/skills')
    : path.join(os.homedir(), '~/.codestudio/skills'.replace(/^~\//, ''));
}
...
class CodestudioTarget implements AgentTarget {
  readonly id = 'codestudio' as const;
  readonly displayName = 'Code Studio';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Code Studio for MCP changes to take effect.'
      ],
    };
  }
...
export const codestudioTarget: AgentTarget = new CodestudioTarget();
