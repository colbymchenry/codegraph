...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'github-copilot.agents/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'github-copilot.agents/skills')
    : path.join(os.homedir(), '~/.copilot/skills'.replace(/^~\//, ''));
}
...
class GithubCopilotTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart GitHub Copilot for MCP changes to take effect.'
      ],
    };
  }
...
export const githubCopilotTarget: AgentTarget = new GithubCopilotTarget();
