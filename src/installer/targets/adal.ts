...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'adal.adal/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'adal.adal/skills')
    : path.join(os.homedir(), '~/.adal/skills'.replace(/^~\//, ''));
}
...
class AdalTarget implements AgentTarget {
  readonly id = 'adal' as const;
  readonly displayName = 'Ada L';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Ada L for MCP changes to take effect.'
      ],
    };
  }
...
export const adalTarget: AgentTarget = new AdalTarget();
