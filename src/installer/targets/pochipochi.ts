...
function configDir(loc: Location): string {
  const targetPath = loc === 'local'
    ? path.join(process.cwd(), 'pochipochi.pochi/skills'.replace(/\/skills$/, ''))
...
function skillsDir(loc: Location): string {
  return loc === 'local'
    ? path.join(process.cwd(), 'pochipochi.pochi/skills')
    : path.join(os.homedir(), '~/.pochi/skills'.replace(/^~\//, ''));
}
...
class PochipochiTarget implements AgentTarget {
  readonly id = 'pochipochi' as const;
  readonly displayName = 'Pochi';
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    return {
      files,
      notes: [
        'Restart Pochi for MCP changes to take effect.'
      ],
    };
  }
...
export const pochipochiTarget: AgentTarget = new PochipochiTarget();
