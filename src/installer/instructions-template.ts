...
/**
 * The body of the codegraph guidance, without the marker fence.
 *
 * Kept separate so targets that own their entire instruction file
 * outright (e.g. Qoder's `.qoder/rules/codegraph.md` — see
 * `targets/qoder.ts`) can write the natural-language content directly,
 * without the HTML comment markers that only make sense for shared
 * files where we co-exist with user content via marker-based upsert.
 */
export const CODEGRAPH_INSTRUCTIONS_BODY = `## CodeGraph
