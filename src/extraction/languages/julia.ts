...
/** Statement nodes in a flat function body (no `block` wrapper). */
const JULIA_BODY_STATEMENT_TYPES = new Set([
  'return_statement',
...
function isJuliaSignaturePart(child: SyntaxNode): boolean {
  if (child.type === 'block' || JULIA_BODY_STATEMENT_TYPES.has(child.type)) {
    return false;
  }
  return (
    child.type === 'signature' ||
    child.type === 'call_expression' ||
    child.type === 'typed_expression' ||
    child.type === 'where_expression' ||
    child.type === 'parameters' ||
    child.type === 'identifier'
  );
}

function visitJuliaFlatFunctionBody(node: SyntaxNode, ctx: ExtractorContext, fnId: string): void {
  let signatureConsumed = false;
  for (let i = 0; i < node.namedChildCount; i++) {
    const child = node.namedChild(i);
    if (!child || child.type === 'block') continue;

    if (!signatureConsumed && isJuliaSignaturePart(child)) {
      signatureConsumed = true;
      continue;
    }

    ctx.visitFunctionBody(child, fnId);
  }
}
...
export const juliaExtractor: LanguageExtractor = {
  ...
  resolveBody: (node, _bodyField) => {
    ...
    // Flat functions/macros are visited in visitNode — never return the definition node
    // itself (visitFunctionBody would re-enter extractFunction and recurse infinitely).
    return null;
  },
  ...
  visitNode: (node, ctx) => {
    ...
    if (node.type === 'function_definition' || node.type === 'macro_definition') {
      const hasBlock = node.namedChildren.some((c) => c.type === 'block');
      ...
      const fn = ctx.createNode('function', name, node, {
        signature: juliaExtractor.getSignature?.(node, source),
      });
      if (!fn) return true;

      ctx.pushScope(fn.id);
      visitJuliaFlatFunctionBody(node, ctx, fn.id);
      ctx.popScope();
      return true;
    }
