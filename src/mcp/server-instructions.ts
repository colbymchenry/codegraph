...
Query module and instance names to inspect hierarchy and the exact port-connection source. Ports/signals, lexical block scopes, generate template parameters, modports and always/assign blocks are searchable. Local signal references belong to their block; calls retain the enclosing function/process. Parser-recovered procedural regions may omit uncertain signal references. Query an instance-qualified connection such as top.u_rx1.rx to inspect its local expression and declared target port; an explicit empty connection still names a formal port but has no local endpoint. Bit/part-select references identify the base signal and indexed expressions, not evaluated widths. HDL references record syntactic signal use, not drive direction, timing or elaborated connectivity; positional bindings require authoritative header order, and wildcard matches require known formal ports and unambiguous local signals (explicit connections take priority). Generate iterations are not expanded. Qualified package calls retain package identity.
...
- **A file flagged "⚠ changed on disk after the last index sync" drifted from its index** (for example when watching is disabled or another process owns live sync). Codegraph never serves a possibly-mis-sliced body from such a file — it either shows the file's full CURRENT source (trust it as a Read) or omits the source with this flag. When the source was omitted, Read that specific file; line numbers referencing it elsewhere in the response may be shifted until that project's next sync. All unflagged files remain trustworthy.
