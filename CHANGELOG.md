...
- **Vue SFC call references are now captured from script setup initializers and Options API methods.** Calls such as `const token = getTokenMp()` in `<script setup>` and `methods: { save() { getTokenMp() } }` in a classic `<script>` block now show up in `codegraph_callers` instead of disappearing from the index. Closes #425.
