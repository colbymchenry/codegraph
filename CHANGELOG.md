...
- All commands now accept a `--data-dir <path>` flag to store CodeGraph data in a custom directory instead of the default `<projectRoot>/.codegraph/`. Useful when you need to keep all tool-generated artefacts under a single directory (e.g. `knowledge/codegraph/`) or when the project root is read-only. Pass an absolute path or a path relative to the project root. The same flag must be passed to every command for a given project. Closes #304, #452.
