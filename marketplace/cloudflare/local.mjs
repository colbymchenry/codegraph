...
export async function startLocal({state,testing=false,boundary,port=0,publishing=true}={}) {
  const mf=new Miniflare({...convertV4MiniflareOptions({host:'127.0.0.1',port,d1Persist:path.join(state,'d1'),r2Persist:path.join(state,'r2'),workers:[{name:'registry',modules:true,scriptPath:path.join(root,'.build',testing?'test-worker.js':'worker.js'),compatibilityDate:'2026-09-21',
