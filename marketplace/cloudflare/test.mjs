...
const root=path.resolve('../..'),out=path.resolve(process.env.CLOUDFLARE_OUTPUT||path.join(root,'.qa/cloudflare/runtime'));fs.mkdirSync(out,{recursive:true});
...
(async()=>{try{
 ...
 await server.bucket.put('packages/sha256/'+published.integrity,Buffer.from('tampered'));assert.equal((await fetch('http://127.0.0.1:'+server.port+'/api/download/worker-demo/1.0.0')).status,503);await server.bucket.delete('packages/sha256/'+published.integrity);assert.equal((await fetch('http://127.0.0.1:'+server.port+'/api/download/worker-demo/1.0.0')).status,503);await server.bucket.put('packages/sha256/'+published.integrity,artifact());passed('missing and corrupt stored artifacts fail closed instead of serving bytes');
 ...
 fs.appendFileSync(path.join(lab,'backup/objects',published.integrity),'corrupt');const empty=await start(path.join(lab,'corrupt-restore'));await assert.rejects(()=>restoreRegistry(empty.db,empty.bucket,path.join(lab,'backup')));assert.deepEqual(await list(empty),[]);passed('corrupt snapshot cannot expose a restored catalog');await restored.close();await empty.close();
 ...
 const large=artifact('max-size','1.0.0',8*1024*1024);assert.equal(large.length,8*1024*1024);const timing=await request(replacement,signed(large));for(let attempt=0;attempt<3;attempt++){const response=await fetch('http://127.0.0.1:'+replacement.port+'/api/download/max-size/1.0.0');assert.equal(response.status,200);const downloaded=Buffer.from(await response.arrayBuffer());assert.equal(hash(downloaded),hash(large));}passed('8 MiB package survives Worker crypto D1 metadata and R2 upload/download');
 ...
 for(const phase of ['after-object','after-commit']){
  ...
  let timer;const reached=new Promise((resolve,reject)=>{timer=setTimeout(()=>reject(Error('No kill checkpoint')),15000);original.c.on('message',m=>{if(m.phase===phase){clearTimeout(timer);resolve();}});});
  ...
  await reached;original.c.kill('SIGKILL');await original.done;await pending;
  const fresh=await start(killstate);assert.equal((await list(fresh)).length,phase==='after-commit'?1:0);assert.equal((await fetch('http://127.0.0.1:'+fresh.port+'/api/download/kill-demo/1.0.0')).status,phase==='after-commit'?200:404);await request(fresh,body,{},phase==='after-commit'?400:201);await fresh.close();
 }
