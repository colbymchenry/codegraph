...
const state = { catalog: [], tab: 'all', search: '', connection: null, snapshot: null, project: '0', popup: null, pending: new Map(), notice: '' };
...
function button(extension, detail = false) {
  ...
  if (current) {
    return `<div class="manage-actions">${current.version !== extension.version ? `<button class="primary" data-action="update" data-id="${escape(extension.id)}">Update to ${escape(extension.version)}</button>` : ''}<button data-action="${current.enabled === false ? 'enable' : 'disable'}" data-id="${escape(extension.id)}">${current.enabled === false ? 'Enable' : 'Disable'}</button>${detail ? `<button data-action="remove" data-id="${escape(extension.id)}">Remove</button>` : ''}</div>`;
  }
  return `<button class="primary" data-action="install" data-id="${escape(extension.id)}">Install extension <span>↓</span></button>`;
}
...
function card(extension) {
  const current = installed(extension.id);
  return `<article class="extension-card"><div class="card-top"><div class="extension-icon">${extension.id === 'drupal' ? icon : escape(extension.name.slice(0, 1))}</div><div><h2><a data-link href="/extensions/${escape(extension.id)}">${escape(extension.name)}</a></h2><div class="byline">${escape(extension.publisher)} ${extension.official ? '<span class="official">✓ OFFICIAL</span>' : '<span>Community</span>'}</div></div><span class="version">v${escape(extension.version)}</span></div><p class="card-copy">${escape(extension.description)}</p><div class="tags">${extension.id === 'drupal' ? '<span class="tag">PHP</span><span class="tag">Drupal 8–11</span>' : ''}${extension.capabilities.map(c => `<span class="tag">${c === 'frameworks' ? 'Framework' : 'Semantic analysis'}</span>`).join('')}</div>${extension.id === 'drupal' ? '<div class="graph" aria-label="Example framework flow: route to controller to service"><span class="graph-node">/your-route</span><span class="graph-line"></span><span class="graph-node blue">Controller</span><span class="graph-line"></span><span class="graph-node">Service</span></div>' : ''}<div class="card-bottom"><a data-link class="text-link" href="/extensions/${escape(extension.id)}">${current ? (current.enabled === false ? 'Disabled · Manage →' : 'Installed · Manage →') : 'Explore extension ↗'}</a>${button(extension)}</div></article>`;
}
...
function renderDetail(id) {
  const e = state.catalog.find(item => item.id === id);
...
async function refresh() { state.catalog = await api('/api/extensions'); render(); }
function connect() {
...
window.addEventListener('message', event => {
  ...
  if (message.type === 'connected' || message.type === 'status') {
    ...
    if (message.type === 'connected') { toast('Connected. Select your project and install an extension.'); render(); }
    else if (JSON.stringify(state.snapshot) !== before && location.pathname !== '/publish') render();
  }
  if (message.type === 'disconnected') disconnect();
});
function disconnect() {
  state.snapshot = null; document.querySelector('#connect').classList.remove('connected');
...
async function act(action, id) {
  ...
  if (action === 'install' || action === 'update') {
    command.url = `${location.origin}/api/download/${extension.id}/${extension.version}`;
    command.integrity = extension.integrity;
...
document.addEventListener('change',async event=>{
  if(event.target.id==='project'){state.project=event.target.value;render();}
...
document.addEventListener('submit',async event=>{
  ...
  try{
    ...
    state.catalog=await api('/api/extensions');toast('Release published. Back up your publisher key to keep control of updates.');navigate('/extensions/'+result.id);
