...
| Haskell | Modules / higher-order functions | entrypoint → imported or re-exported action; reader → AST → writer | R + X | 🔬 **module-aware extraction and resolution audited 2026-09-03** — xmonad v0.18.1 S: **3/3** targeted flows complete; pandoc 3.11 M: **3/3** complete; HLS L: the HLint flow completes in one explore, while startup and hover stop at function-valued parameters passed through helpers. Re-export deduplication/indexing/compaction makes HLS complete under the watchdog (**>449s without a first persisted batch → 64.59s total; Compat/Core.hs 20,277 → 455 routes**) without pending refs. The remaining hole needs a whole-graph call-through summary that proves which parameters are invoked and propagates that proof through resolved calls; promoting every named function argument would create false calls for stored/registered handlers. Official multi-run Sonnet A/B remains pending. **[2026-09-13 follow-up audit](../benchmarks/haskell-audit-2026-09-13.md):** ra-monorepo 3/3 selected flows remain complete in one explore; nine xmonad/pandoc/HLS control queries preserve their previous chains, including documented gaps; 47 duplicated ra selectors removed and constructor-qualified field references corrected; 168 Haskell tests pass on macOS/Linux, with the full suites retaining the documented upstream failures. |
