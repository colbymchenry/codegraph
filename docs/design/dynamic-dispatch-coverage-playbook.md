...
| Verilog / SystemVerilog | — (HDL, no framework) | **module instantiation hierarchy** (top module → leaf module, the HDL equivalent of a call path) | X | ✅ **new language** (`.v/.vh/.sv/.svh`, vendored `tree-sitter-systemverilog` ABI 15). Extractor (`languages/verilog.ts`, custom `visitNode` hook — SV nests names deep in `*_ansi_header`/`*_body_declaration`) emits modules/interfaces/packages (→class/interface), functions/tasks, params (→constant), typedefs, package imports, **and `module_instantiation` → `instantiates` edges** — the high-value HDL relationship. **Plus a cross-cutting traversal change: `codegraph_trace`/`callers`/`callees` now follow `instantiates`** (precise tree-sitter edge), so the instantiation hierarchy is queryable — without it the edges existed but no flagship tool walked them (`impact` already traversed all edges). Extraction (deterministic probes): picorv32 (`.v`, S) **61 modules / 101 inst edges**, trace `picorv32_axi→picorv32_pcpi_mul` **3 hops**; ibex (`.sv`, L) **610 modules / 24 interfaces / 279 typedefs / 559 inst edges**, trace `ibex_top→ibex_decoder` **4 hops**; cva6 (`.sv`, L) **658 / 22 / 541 / 625**, trace `cva6→load_store_unit` **3 hops**. ⚠️ **Agent A/B is provisional — NOT the canonical benchmark**: run on **DeepSeek `deepseek-v4-pro`, n=1 per arm** (not Opus 4.7 × median-of-4), so it does **not** qualify for the README table / `codegraph-ab-matrix.md` — re-run with the standard methodology before publishing those. Directional deltas (with→without): picorv32 **2→12 tool calls, 0→7 Read+Grep, 16s→64s**; ibex **1→12, 0→7, 15s→57s**; cva6 **1→13, 0→8, 15s→76s** — every with-arm answered the hierarchy with **0 Read/0 Grep in 1–2 codegraph calls**; both arms always correct. 🔬 ports/internal signals deliberately not extracted (dataflow frontier — would explode node count); SV interfaces/modports as typed connection channels, `generate`-block instances, and param-value dataflow are uncovered |
