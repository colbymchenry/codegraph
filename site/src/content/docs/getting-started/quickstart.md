...
Auto-detects and configures Claude Code, Cursor, Codex CLI, opencode, Hermes Agent, Gemini CLI, Antigravity IDE, Kiro, and GitHub Copilot — wiring the CodeGraph MCP server into each. For pi (which has no MCP client) it writes a CLI-oriented CodeGraph section into `~/.pi/agent/AGENTS.md` instead. This step connects your agents only; it does **not** index any code. (Shortcut: `npx @colbymchenry/codegraph` downloads and runs the installer in one go.)
